...
//  int addNetCDFVar(CImg &img,std::string var_name,std::string unit_name,std::vector<std::string> dim_names,std::string time_name)
  ...
  std::cerr << "CImgNetCDF::" << __func__ << "(CImg<" << img.pixel_type() << ">," << var_name << "," << unit_name << ")" << std::endl;
    ...
    {
      switch(i){
        case 3:if(loadDim(i)!=img.dimv()) {return DIM_ERROR;}break;
        case 2:if(loadDim(i)!=img.dimz()) {return DIM_ERROR;}break;
        case 1:if(loadDim(i)!=img.dimy()) {return DIM_ERROR;}break;
        case 0:if(loadDim(i)!=img.dimx()) {return DIM_ERROR;}break;	
        default:NULL;
      }
    }
...
std::cerr<<__func__<<" unlimited direction="<<time<<std::endl;
  ...
  std::cerr<<__func__<<" no unlimited direction"<<std::endl;
          ...
          switch(vpNCDim.size())
          {
            ...
            case 4:if (!pNCvar->put(ptr,img.dim,img.depth,img.height,img.width)) return NC_ERROR;break;
            default:return CODE_ERROR;
          }
        }
        else
  ...
  std::cerr << time_name << ")" << std::endl;
    ...
    for(int i=0;i<dim_names.size();i++)
    {
      switch(i){//Write or use existing
        case 0:if(error=addNetCDFDim(dim_names[i],img.dimx(),pd)) return error;break;
        case 1:if(error=addNetCDFDim(dim_names[i],img.dimy(),pd)) return error;break;
        case 2:if(error=addNetCDFDim(dim_names[i],img.dimz(),pd)) return error;break;
        case 3:if(error=addNetCDFDim(dim_names[i],img.dimv(),pd)) return error;break;
        default:NULL;
      }
      vpNCDim.push_back(pd);
    }
