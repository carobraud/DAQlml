...
/*  template<typename T> T* ptr(void)
  ...
  **/
  ...
  std::cerr << "," << ((pTime_name==NULL)?"don't ":"") <<"search for unlimited dimension"<< ")" << std::endl<<std::flush;
    ...
    if (!(pNCDimv=pNCvar->get_dim(i++))) return NC_ERROR;//Read
    if (!(pNCDimz=pNCvar->get_dim(i++))) return NC_ERROR;//Read
    if (!(pNCDimy=pNCvar->get_dim(i++))) return NC_ERROR;//Read
    if (!(pNCDimx=pNCvar->get_dim(i++))) return NC_ERROR;//Read
    dim_names.clear();
    dim_names.push_back(std::string(pNCDimx->name()));
    dim_names.push_back(std::string(pNCDimy->name()));
    dim_names.push_back(std::string(pNCDimz->name()));
    dim_names.push_back(std::string(pNCDimv->name()));
//#if cimg_debug>10
...
//#endif
    //dims can be now loaded using loadDim[x,y,z,v,Time]() methods
    //check requested number of dimensions //###
    if(dim_names.size()!=4) {std::cerr << "CImgNetCDF::" << __func__ << " Error: actually variable needs to be 4D." << std::endl;return NC_ERROR;}
    return 0;
  }
