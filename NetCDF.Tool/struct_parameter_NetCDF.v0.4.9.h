...
#if cimg_debug>2
    std::cerr << "CParameterNetCDF: check types " << (NcTypeInfo<T>::ncId()) << "==" << (var->type()) << std::endl<< std::flush;
#endif
    if( (NcTypeInfo<T>::ncId())==(var->type()) )//check if same type
    {
#if cimg_debug>10
      std::cerr << "CParameterNetCDF: get" << std::endl<< std::flush;
#endif
      if (!(var->get(&value))) return NC_ERROR;
    }
    else//convert to same type
    {
std::cerr << "CParameterNetCDF: NOT get" << std::endl<< std::flush;
#if cimg_debug>10
      std::cerr << "Information/CParameterNetCDF: convert <" << NcTypeStr(var->type()) << ">" << var_name << " to <" << (NcTypeInfo<T>::ncStr()) << ">" << std::endl<< std::flush;
//! \todo Warning: convert \c NcType to \c T type (actually only \c as_int is implemented)
std::cerr << "Warning/CParameterNetCDF: only read T=int as only 'as_int()' is implemented !!" << std::endl<< std::flush;
#endif
